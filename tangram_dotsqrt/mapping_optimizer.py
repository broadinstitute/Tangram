...
class Mapper:
    ...
    def _loss_fn(self, verbose=True):
        ...
        gv_term = self.lambda_g1 * torch.mul(torch.sqrt(G_pred), torch.sqrt(self.G)).mean()
        #gv_term = self.lambda_g1 * cosine_similarity(G_pred, self.G, dim=0).mean()
        vg_term = self.lambda_g2 * torch.mul(torch.sqrt(G_pred), torch.sqrt(self.G)).mean()
